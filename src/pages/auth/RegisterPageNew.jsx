...
export const RegisterPageNew = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (requiresApproval) {
        // IMPORTANT: Always show a clear success screen for approvals (claim, verification, etc.)
        // so users don't feel like "nothing happened".
        ...
        setSuccessMessage(
          result?.message ||
            (claimUser
              ? 'Profil sahiplenme başvurunuz alındı. İnceleme sonrası bilgilendirileceksiniz.'
              : 'Başvurunuz alındı. İnceleme sonrası bilgilendirileceksiniz.')
        );
        return;
      } else {
  ...
  const renderChoice = () => (
    <div className="space-y-6">
      <div className="grid gap-4">
        ...
        >
          <div className="w-24 h-24 bg-blue-100 rounded-full flex items-center justify-center mr-6 group-hover:scale-110 transition-transform">
        ...
        >
          <div className="w-24 h-24 bg-purple-100 rounded-full flex items-center justify-center mr-6 group-hover:scale-110 transition-transform">
      ...
      <div className="w-full max-w-2xl bg-white rounded-3xl shadow-2xl p-6 md:p-10 border border-gray-100">
        ...
        {step === 1 && (
            <div className="space-y-6">
              ...
              <div className="relative">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-7 h-7 text-gray-400" />
                <input
                  type="text"
                ...
                {MEMBERSHIP_TYPES.map((type) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center p-4 bg-white border-2 border-gray-100 rounded-xl transition-all group text-left ${type.borderColor} hover:shadow-md`}
                    >
              <div className={`w-24 h-24 rounded-2xl flex items-center justify-center mr-5 flex-shrink-0 shadow-lg transform group-hover:scale-110 transition-transform duration-300 ${type.iconBg}`}>
